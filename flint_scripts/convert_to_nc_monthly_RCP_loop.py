...
for folder in folders.keys():
    ...
    if os.access(unzip_to+folders[folder]+'/',os.F_OK)==False and os.access(unzip_to+folder,os.F_OK)==False:
       subprocess.call('tar -zxvf {0}{1}.tar.gz -C {2} {3}/Monthly/*'.format(src,folder,unzip_to,folders[folder]),shell=True)
    
    if os.access(unzip_to+folder,os.F_OK)==False:
        subprocess.call('mv {0} {1}'.format(unzip_to+folders[folder].replace(' ','\ '),unzip_to),shell=True)
    ...
    subprocess.call('rm -r '+unzip_to+folder,shell=True)
