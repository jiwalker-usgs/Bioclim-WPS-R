...
daily_ncml_2=' regExp=".*(?&lt;!monthly)\.\d{4}.*" subdirs="false"/>\n </aggregation>\n</netcdf>'
...
grids=('BCCA_0.125deg','OBS_125deg')
calendars=['gregorian','obs']
                                        ...
                                        if line[0]=='.': #if line is a path, do some union stuff if the path is terminal.
                                                ...
                                                if path_switch==1: # If the last line was a path, write two join files, monthly and daily.
                                                        ...
                                                        if grid in line and calendar in path and time_prd in path and run in path:
                                                                ...
                                                                if 'pr' in path:
                                                                        var_name=line[0:-1-13].replace('.','-') # create unique name for variable.
                                                                        name=line[0:-1-13]+'.ncml' # create name of join file.
                                                                        orgName='pr'
                                                                if 'tasmax' in path:
                                                                        orgName='tasmax'
                                                                        var_name=line[0:-1-8].replace('.','-') # create unique name for variable.
                                                                        name=line[0:-1-8]+'.ncml' # create name of join file.
                                                                if 'tasmin' in path:
                                                                        orgName='tasmin'
                                                                        var_name=line[0:-1-8].replace('.','-') # create unique name for variable.
